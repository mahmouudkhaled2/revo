...
import { doc, updateDoc, collection, addDoc, deleteDoc, Timestamp, arrayUnion, arrayRemove, getDoc } from 'firebase/firestore';
...
export default function Post({ post }) {
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      // Add comment to Firebase subcollection first to get the ID
      ...
      // Update local state with the comment that has an ID
      ...
      return (
        <div className="mt-4 border-t pt-4">
          <p className="text-gray-600 text-sm mb-3">
            Want to like or comment on this post? Login to join the conversation!
          </p>
          <button
          ...
          {comments?.length > 0 && (
            <div className="mt-4 border-t pt-3">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-sm">
                    <span className="font-semibold">{comments[0]?.userName}</span>
                    {" "}{comments[0]?.text}
                  </p>
                  <p className="text-xs text-gray-500 mt-1">
                    {formatDistanceToNow(comments[0]?.createdAt?.toDate(), { addSuffix: true })}
                  </p>
                </div>
              </div>
              {comments.length > 1 && (
